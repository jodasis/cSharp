...
namespace LibraryCompilePepe
{
    public class PCC
    {
        ...
        //Debemos definir un método que reciba el archivo de texto y de este extraiga la lista de strings
        public void recibeTransformaArchivo(String rutaArchivo)
        {
            //ocurre mágia
            ...
            using (StreamReader archivo = new StreamReader(rutaArchivo))
            {
                while (!archivo.EndOfStream)
                    programaPEPE.Add(archivo.ReadLine());
            }
        }
            ...
            foreach (string linea in programaPEPE)
            {
                ...
                switch (tokens[1])
                {
                    ...
                    case "printLine":
                        ...
                        programaCompilado.Add(posicionDisponibleInstruccion++, "1400");
                        break;
                    case "readCadena":
                        //es sencillo: solo se pone la señal de fin
                        programaCompilado.Add(posicionDisponibleInstruccion++, "121A");
                        break;
                    case "WriteCadena":
                        //es sencillo: solo se pone la señal de fin
                        programaCompilado.Add(posicionDisponibleInstruccion++, "131A");
                        break;
                    case "resultInt":
                        //es sencillo: solo se pone la señal de fin
                        programaCompilado.Add(posicionDisponibleInstruccion++, "5000");
                        break;
                    case "resultDecimal":
                        //es sencillo: solo se pone la señal de fin
                        programaCompilado.Add(posicionDisponibleInstruccion++, "5100");
                        break;
                    case "end":
                        //es sencillo: solo se pone la señal de fin
        ...
        public void exportarSLM(string ubicacion)
        {
            string[] programaSLM = new string[100];
            for (int i = 0; i < programaSLM.Length; i++)
                programaSLM[i] = "0000";
            foreach(int i in programaCompilado.Keys)
                programaSLM[i] = programaCompilado[i];
            ...
            using (StreamWriter sw = new StreamWriter(fs))
                foreach (string word in programaSLM)
                    sw.WriteLine(word);
